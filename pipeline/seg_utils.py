# # seg_utils.py
# """
# Utilities: YOLO-seg → polygon → oriented bounding-box → perspective crop
# """
...
# class Segmenter:
#     """Wrap a YOLO-v8 segmentation checkpoint."""
#     def __init__(self, ckpt: str, imgsz: int = 640, conf: float = 0.1,
#                  device: str = "cuda"):
...
#     def segment(self, img_path: str, keep_cls: Optional[set[int]]=None
#                 ) -> List[Tuple[np.ndarray, int]]:
#         """
#         Returns list of (polygon Nx2 float32 , class-id)
#         """
...
#         cids  = results.boxes.cls.cpu().numpy().astype(int)
#         out   = []
#         for p,c in zip(polys, cids):
#             if keep_cls and c not in keep_cls: continue
#             out.append((np.asarray(p, np.float32), c))
#         return out


# def polygon_to_obb(poly: np.ndarray) -> np.ndarray:
#     """poly Nx2 → 4×2 int (min-area rectangle corners)."""
...
# def save_mask_or_box(
#     img_shape: tuple, poly: np.ndarray, box: np.ndarray,
#     save_mask: Optional[str]=None, save_box: Optional[str]=None
# ):
#     """Optional helpers to write mask.png / box.txt for debugging."""
#     if save_mask:
#         mask = np.zeros(img_shape[:2], np.uint8)
#         cv2.fillPoly(mask, [poly.astype(int)], 255)
#         cv2.imwrite(save_mask, mask)
#     if save_box:
#         np.savetxt(save_box, box, fmt="%d")
